...
\ own xt (known as pxt). When pr-str is called, the here value is pushed
\ onto the data stack, and then the does> part is executed, which
\ fetches the pxt and then calls execute-method to look up and execute
\ the correct implementation (ixt) for the object type and pxt.
\ Note the def-protocol-method example above could be written:
\   : pr-str ( obj -- str ) [ latestxt ] literal execute-method ;

\ : def-protocol-method ( "name" -- )
\     create latestxt ,
\   does> ( ??? obj xt-ref -- ??? )
\     @ [ call-site% %allocate throw ] literal execute-method ;
...
: def-protocol-method ( parse: name -- )
    ...
    \ ." Define proto " latestxt dup . >name name>string type cr
;
